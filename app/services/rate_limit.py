...
class RateLimitService:
    ...
    async def can_generate(self, user_id: int) -> tuple[bool, str | None]:
        if await self._key_value_store.exists(self._generation_active_key(user_id)):
            return False, GENERATION_ACTIVE_MESSAGE

        daily_count = await self._key_value_store.get(self._generation_daily_key(user_id))
        if daily_count is not None and int(daily_count) >= self._daily_generation_limit:
            return False, GENERATION_DAILY_LIMIT_MESSAGE

        if await self._key_value_store.exists(self._generation_cooldown_key(user_id)):
            return False, GENERATION_COOLDOWN_MESSAGE

        return True, None

    async def mark_generation_started(self, user_id: int) -> None:
        await self._key_value_store.setex(
        ...
        daily_count = await self._key_value_store.incr(daily_key)
        if daily_count == 1:
            await self._key_value_store.expire(daily_key, self._seconds_until_next_utc_day())

    async def mark_generation_finished(self, user_id: int) -> None:
        await self._key_value_store.delete(self._generation_active_key(user_id))

    async def can_contact_support(self, user_id: int) -> tuple[bool, str | None]:
        if await self._key_value_store.exists(self._support_cooldown_key(user_id)):
